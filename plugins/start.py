...
async def start_command(client: Client, message: Message):
    ...
    if id == owner_id:
        ...
        if "verify_" in message.text:
            ...
            await message.reply("Your token successfully verified and valid for 24 hours.")
            ...
            if len(argument) == 3:
                try:
                    ...
                    return
                ids = range(start, end + 1) if start <= end else list(range(start, end, -1))
            elif len(argument) == 2:
                try:
...
# Command handler for users not joined the required channel
